...
int initializationpm(int argc, char *argv[]){
    if(argc < 2){
        printf("use lithium init\n");
        return 0;
    }
    system("mkdir -p /lithium/pkgs/");
    sleep(1);
    system("mkdir -p /lithium/pm");
    return 0;
}
int instl(int argc,char *argv[]){
    ...
    if(yorn == 'y'){
        for(int i=2; i<argc;i++){
            ...
            snprintf(ttyi,sizeof(ttyi),"wget -q --directory=/lithium/pkgs/ %s%s",repository,argv[i]);
            snprintf(lns,sizeof(lns),"ln -s /lithium/pkgs/%s /bin/%s",argv[i],argv[i]);
            snprintf(chmd,sizeof(chmd),"chmod +x /lithium/pkgs/%s /bin/%s\n",argv[i],argv[i]);
            system(ttyi);
            sleep(1);
            system(lns);
            sleep(1);
...
int rmv(int argc,char *argv[]){
    ...
    if(yorn == 'y'){
        for(int i=2; i<argc;i++){
            ...
            snprintf(ttyr,sizeof(ttyr),"rm -rf /lithium/pkgs/%s /bin/%s",argv[i],argv[i]);
            system(ttyr);
        }
    }
    if(yorn == 'n'){
...
int upd(int argc, char *argv[]){
    ...
    if(yorn == 'y'){
        ...
        sleep(1);
        system("wget -q --directory=/lithium/pm/ https://raw.githubusercontent.com/wholos/LithiumPackages/main/lithium/lithium");
        sleep(1);
        system("ln -s /lithium/pm/lithium /bin/lithium");
        sleep(1);
        system("chmod +x /lithium/pm/lithium /bin/lithium");
    }
    if(yorn == 'n'){
        printf("Aborting.\n");
    }
...
int main(int argc,char *argv[]){
    ...
    if(strcmp(argv[1],"--help")==0){
        ...
        printf("init: initalization lithium to /mnt\n");
